...
if (
  ...
  empty($_POST["repeatDay"]) ||
  empty(trim($_POST["repeatWeek"]))
  ...
  $repeatWeek = trim($_POST["repeatWeek"]);
  ...
  if ($stmt = mysqli_prepare($link, $sql)) {
    ...
    if (count($repeatDay) == 1 && $repeatDay[0] ==  "1") {
      ...
      for ($j = 0; $j < count($repeatDay); $j++) {
        ...
        $timestamp = strtotime($final_start_time);
        ...
        for ($k = 0; $k < $z; $k++) {
          ...
          $final_start_time = date('Y-m-d', strtotime($final_start_time . ' next ' . $repeatDay[$j]));
          $final_start_time = date('Y-m-d', strtotime($final_start_time . ' + ' . $k * 7 . ' days'));
