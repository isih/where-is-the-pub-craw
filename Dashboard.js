...
function suggestStop(el, num, city) {
    ...
    let superAdminSelectedCity = document.getElementById("city");// if the super admin is logged in we need to retrieve the name of the city that he choose to create a route.
...
setInterval(function todayOrWeekly() {
    let todayOrWeekly = document.getElementsByName('repeatRouteChoice');
    let repeatRouteChoiceDay = document.getElementsByName('repeatRouteChoiceDay');
    let dropdownWeeks = document.getElementsByName('weekItem');
    let justToday = false;
    if (todayOrWeekly[0].ariaChecked == "true") {
        justToday = true;
    }
    enableDisable(justToday, repeatRouteChoiceDay, dropdownWeeks)
}, 500)

function enableDisable(justToday, repeatRouteChoiceDay, dropdownWeeks) {
    if (justToday === true) {
        for (i = 0; i < repeatRouteChoiceDay.length; i++) {
            repeatRouteChoiceDay[i].disabled = 'true'
        }
        dropdownWeeks[0].disabled = 'true';
    } else {
        for (i = 0; i < repeatRouteChoiceDay.length; i++) {
            repeatRouteChoiceDay[i].disabled = 'false'
        }
        dropdownWeeks[0].disabled = 'false';
    }
}

function suggestCity(el) {
    coordinates = [];
    let suggestions = [];
    let content = el.value;
    let target = document.getElementById("city-suggestions");
    ...
    //retrieveing suggestions:
    $.ajax({
        url: 'phpUtilities/getSuggestionsCity.php',
        type: 'POST',
        ...
        success: (output) => {
            ...
            suggestions.forEach(element => {
                suggestionsHTML += "<ion-item>";
                suggestionsHTML += "<ion-label onclick='selectSuggestionCity(this)'>" + element + "</ion-label>";
                suggestionsHTML += "</ion-item>";
            });
            target.innerHTML = suggestionsHTML;
        }
...
function selectSuggestionCity(el) {
    let content = el.innerHTML;
...
function confirmedPubCrawlPlanning() {
    ...
    if (todayOrWeekly[1].ariaChecked == "true") {
        let repeatRouteDay = document.getElementsByName('repeatRouteChoiceDay');
        for (i = 0; i < repeatRouteDay.length; i++) {
            if (repeatRouteDay[i].ariaChecked === "true") {
                repeatDay.push(repeatRouteDay[i].value)
            }
        }
        console.log("repeatDay: " + repeatDay)

        let repeatRouteWeeks = document.getElementsByName("weekItem");
        repeatWeek = repeatRouteWeeks[1].value;
        console.log("repeatWeek: " + repeatWeek)
    }
    else {
        repeatDay.push(1);
    }
    ...
    */
    if (!ok || datetimeInput === '' || targetDuration === '' || targetMeetingPoint === '' || targetCity === '' || repeatDay === undefined || isNaN(parseInt(targetDuration)) || (parseInt(targetDuration) * 60) < parseInt(totalDuration)) {
        ...
        if (repeatDay.length == 1 && repeatDay == "1") {
            ...
            if (repeatWeek === "") {
                alert.header = 'Check your input!';
                alert.subHeader = "We couldn't schedule this Pubcrawl";
                alert.message = "You need to select for how many weeks you want this pubcrawl route to be created.";
                alert.buttons = ['Ok'];
            } else {
                alert.message += '<br><b>Repeat this route every: </b><br>' + repeatDay + '. <br><b>For the next: </b><br>' + repeatWeek + ' weeks.';
